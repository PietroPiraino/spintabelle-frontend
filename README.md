...
Sezioni: `/tabelle` (viewer preflop GTO, stato in query param) · `/lezioni` (video gated paginati) · `/allenamento` (drill) · `/simulatore-varianza` (**pubblico**, simulatore Monte Carlo di varianza per Spin & Go / Twister) · `/live` + `/live/:id/stanza` (**lezioni dal vivo on-site**) · `/docs` (file scaricabili) · `/guide` + `/guide/:slug` (**10 guide evergreen**, pubbliche e prerenderizzate: ⚠️ il contenuto vive **strutturato** in `guides.data.ts`, non in Markdown, così gli `<h2>` sono reali nell'HTML statico — aggiungerne una è una voce nell'array, e gli slug del prerender si leggono da quel file, mai da un'API) · `/news` + `/news/:slug` (**rese all'edge**, non prerenderizzate — vedi *SEO & SSG*) · `/redazione` e `/policy-editoriale` (le due pagine che sostengono la trasparenza della redazione) · `/abbonati` (pubblica) · `/negozio` (punti) · `/affiliazioni` (**pubblica ma col catalogo dietro login**, LIVE dal 02/08/2026: quadro di controllo delle proprie pratiche presso le poker room — vedi sotto) · `/account` · `/chi-siamo` · `/admin` (**dashboard a sidebar**, non più a tab: ogni sezione è una rotta figlia `/admin/<sezione>` — panoramica · lezioni · live · **news** · **redazione** · **fonti** · documenti · negozio · iscritti · richieste · sconti · **affiliazioni** · partecipazione · **replayer** · stakings · conteggi-mensili · statistiche · log (l'ordine della sidebar; nessun segnaposto: `conteggi-mensili` è viva dal 10/09/2026, e **Panoramica e Statistiche sono state riscritte il 13/09/2026** — la prima legge il cruscotto dei conteggi e porta a ogni sezione, la seconda è a quattro schede Abbonati · Incassi · Andamento · Video col grafico a colonne condiviso `<app-grafico-colonne>`). ⚠️ **News e Redazione sono due schermate diverse**: Redazione è la **coda** e mostra solo gli `IN_REVISIONE`; News è l'**archivio** di ogni stato, ed è l'unica che elenca i pubblicati — quindi è lì che stanno «Genera copertina», «Storia IG» e «Annuncia su Discord» — dall'08/09/2026 dentro la **scheda dell'articolo**, che si apre col `⋮` della riga).
...
**SSG attivo dal 12/07/2026** (`outputMode: 'static'`, prerender al build — **nessun server SSR a runtime**): **27 pagine** prerenderizzate (17 rotte fisse + una per guida) (`app.routes.server.ts`), con meta/canonical/OG/JSON-LD **nell'HTML grezzo** → anteprime social per-pagina.

⚠️ **Le news NON sono più fra queste, dal 19/08/2026**: `/news` e `/news/:id` le compone a ogni richiesta una **Cloudflare Pages Function** (`functions/news/[[path]].ts` + `functions/lib/render-news.mjs`), perché gli articoli si pubblicano dal pannello **senza deploy**. In `app.routes.server.ts` quelle due rotte sono `RenderMode.Client` — controintuitivo e da leggere prima di «correggerlo»: l'HTML iniziale non lo fa Angular, lo fa la Function, e il render mode governa solo la navigazione interna allo SPA. ⚠️ **Mai `_worker.js`**: lì `_headers`/`_redirects` smetterebbero di applicarsi e cadrebbe la deindicizzazione delle rotte client, in silenzio.

`npm run build` = **`ng build && node scripts/inject-csr-noindex.mjs && node scripts/gen-sitemap.mjs && node scripts/check-routes.mjs && node scripts/check-prerender-content.mjs`** — iniezione del `noindex` nella shell CSR, sitemap *derivata* dal manifest, **guardia rotte** e **guardia contenuto** (apre l'HTML e controlla parole, un solo `<h1>`, canonical, `noindex` solo dove dichiarato). ⚠️ `scripts/check-news-live.mjs` è la metà news della guardia ed è **volutamente FUORI** dalla catena: punta alla **produzione**, non a `dist/`, quindi in catena farebbe dipendere ogni deploy dallo stato del precedente. Va lanciato **a mano dopo ogni deploy che tocca le news** — è una delle tre sonde che girano contro la PRODUZIONE e non su `dist/` — con `check-hands-live.mjs` (le pagine delle mani, anch'esse rese all'edge) e `check-admin-tabelle.mjs` (le tabelle del pannello admin, che vuole invece uno stack locale acceso e dei dati).

A runtime `core/services/seo.service.ts` applica i meta per-pagina (listener `NavigationEnd` in `app.config.ts`); `news-detail` li ridefinisce coi dati veri dell'articolo.
...
**Rebuild automatico alla pubblicazione news** — ⚠️ **e questo paragrafo diceva il falso fino al 23/08/2026**: sosteneva che un articolo pubblicato dall'admin non fosse visibile finché non si rideployava. Da quando le due pagine news le compone la Pages Function (19/08/2026) **l'articolo esiste appena è pubblicato, e nessun build lo cambia**. Il **Cloudflare Deploy Hook** (`DeployHookService`, vedi `../CLAUDE.md`) resta acceso per un motivo diverso e più piccolo: la **home è prerenderizzata** e stampa gli ultimi 3 articoli. Perciò parte su **cinque** transizioni — quelle che cambiano la home — e **non** su `rettifica` o `rinomina-slug`. Comportamento atteso: **una pubblicazione fa partire un build, una rettifica no**; l'assenza del build dopo una rettifica non è un guasto.
